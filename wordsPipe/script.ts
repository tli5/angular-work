...
class CleanPipe {
  ...
  transform(sentence : string, dirtyWordsStr : string) : string {
    let dirtyWords = dirtyWordsStr.split(",");
    let words : [] = sentence.split(" ");
    
    for (let i = 0; i < words.length; i++)
    {
      for (let dirtyWord of dirtyWords)
      {
        if (words[i] == dirtyWord)
        {
          console.log("replacing word " + words[i]);
          words[i] = "[" + words[i] + "]";
        }
      }
    }
...
<div class="card card-block">
  ...
  <p class="card-text"
     [hidden]="data.hide">{{data.punchline | clean:'boo,hell,hell'}}</p>
  <a (click)="data.toggle()"
...
class JokeListComponent {
  ...
  constructor() {
    this.jokes = [
      new Joke("boo hell normal?", "damn normal"),
